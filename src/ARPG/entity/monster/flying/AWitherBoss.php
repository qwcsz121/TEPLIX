...
use ARPG\entity\projectile\ABlueWitherSkull;
...
class AWitherBoss extends FlyingMonster implements ProjectileSource{
 const NETWORK_ID = 52;
 ...
 public function attackEntity(Entity $player){
  if($this->attackDelay > 20 && mt_rand(1, 32) < 4 && $this->distance($player) <= 18){
   ...
   $pk = new ABlueWitherSkull($this->level, $nbt, $this);
   $pk->setExplode(true);
 ...
 public function getDrops(){
  if($this->lastDamageCause instanceof EntityDamageByEntityEvent){
   return [Item::get(399, 0, mt_rand(0, 1))];
  }
  return [];
 }
